        ...
        <div className="main-inner">
          ...
          {run && !detailLoading && (
            <>
              ...
              <div className="page-intro"><div><h1>Make the invisible<br /><em>inspectable.</em></h1><p>Good answers are not enough. Follow the trace, check the constraints, then decide what should happen next.</p></div><div className="intro-meta"><span className="meta-eyebrow">THIS REVIEW</span><strong>{run.title}</strong><span>{formatTime(run.createdAt)}</span><span className="sample-pill">SYNTHETIC TRACE</span></div></div>
              ...
              {failedCount > 0 && (diagnosis || proposal) && <section className="review-step" aria-labelledby="review-step-title" ref={proposalRef} tabIndex={-1}>
                ...
                {proposal && <div className="proposal-card"><div className="proposal-topline"><span>PROPOSAL {String(proposal.version).padStart(2, "0")}</span><span className={`proposal-status status-${proposal.status}`}>{proposal.status.toUpperCase()}</span></div><h3>What should change next</h3><p className="proposal-task">{proposal.proposedTask}</p><p className="proposal-source">Source: deterministic demo rules · Created {formatTime(proposal.createdAt)}</p>
